...
if ($::HKEY_CURRENT_USER->Open("Control Panel\\Desktop", $key)) {
  ...
  #if ($ScreenSaveActive) {$key->SetValueEx("ScreenSaveActive", $reserved, $type, '0');}
} 
$iexplore = 'iexplore';
...
our $Tk = ($mbrfolder && -e "$mbrfolder/phoplayer.exe") ? 3 : 1;
our $Hk = 0;
our $Ck = 0;
our $ordostatus = 0;
our $missa = 0;
...
if ($ordostatus) {ordo();}
elsif ($command && ($votive !~ /C9/i || $command !~ /(Prima|Tertia|Sexta|Nona|Completorium)/i)) {
...
for ($i = 1; $i < 9; $i++) {
  my $item = $horas[$i];
  if ($votive =~ /Defunctorum/i && $i > 2 && $i != 7) {next;} 
...
if ($missaprog) {
  ...
  if ($mp && $mp =~ /\?/) {$mp = $`;}
  if (-e "$missadir/$mp") {
    $bframe2->Button(-text=>'Sancta Missa', -background=>$framecolor, -borderwidth=>0,  -foreground=>$blue,
      -command=>sub{
...
foreach ('Versions', 'Credits', 'Rubrics', 'Help', 'Controls') {
  ...
  $bframe3->Button(-text=>$item, -background=>$framecolor, -borderwidth=>0, -font=>$labelfont, 
    -command=>sub{system("start $iexplore \"$datafolder/Help/$item1.html\"");}) 
    ->pack(-side=>'left', -padx=>$padx);  
}
...
if ($voice && $phoplayer && $Tk > 2 && $lang1 =~ /Latin/i) 
  {push(@optarray6, 'chant Latin #1');}
if ($voice && $phoplayer && $Tk > 2 && $lang2 =~ /Magyar/i) 
  {push(@optarray6, 'chant Magyar #2');}
