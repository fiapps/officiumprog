...
sub getweek {
   ...
   if ($missa) {return sprintf("Epi%1i", 8 - $wdist);}
 ...
 if (exists($transfer{"Hy$sday"})) {$hymncontract = 1;}
  ...
  if (-e "$datafolder/$lang1/$tn.txt" || $dayname[0] =~ /Epi0/i) {  
     ...
     if ($hora =~ /(vespera|completorium)/i) {      
	   ...
	   if (exists($transferspec{$tn1})) {$tn1 =$transferspec{$tn1};}
     elsif(exists($transfer{$tn1})) {$tn1 = $transfer{$ntday};}   
     ...
     $trank = $tempora{Rank};     
     if ($hora =~ /(Vespera|Completorium)/i && $tempora{Rule} =~ /No secunda Vespera/i) 
  ...
  if (-e "$datafolder/$lang1/$sn.txt") { 
   ...
   $srank = $saint{Rank}; 
   if ($hora =~ /(Vespera|Completorium)/i && $saint{Rule} =~ /No secunda Vespera/i) {$srank = ''; %saint = undef;}
  } else {$srank = '';}                 
  ...
  if ($hora =~ /(vespera|completorium)/i) {     
	...
	if (($cday =~ /10\-DU/i || !transfered($ocday)) && -e "$datafolder/$lang1/$cday.txt") {  
      $cname = "$cday.txt";             
	  ...
	  if ($crank =~ /(Feria|Vigilia)/i) {$crank = '';}
	} 
    %crank = split(";;", $crank);     
  ...
  if ($version !~ /monastic/i && $dayname[0] !~ /(Adv|Quad[0-6])/i && $dayname[0] !~ /Quadp3/i && $testmode !~ /^season$/i) { 
    if ($dayofweek == 6 && $srank !~ /Vigil/i && $trank[2] < 2 && $srank[2] < 2 && !$transfervigil) { 
	...
	if ($hora =~ /(Vespera|Completorium)/i && $dayofweek == 5 &&  $crank !~ /;;[2-7]/ && $srank !~ /;;[5-7]/ &&
        $crank !~ /Vigil/i && $version !~ /1960/  && $saint{Rule} !~ /BMV/i) { 
  ...
  if ($srank[2] && $srank[2] > $trank[2] && 
   ...
   if ($hora !~ /Completorium/i) {$dayname[2] = "Transfer $trank[0]";}
    ...
    if (($hora =~ /matutinum/i || !$dayname[2]) && $rank < 7) { 
  ...
  if (transfered($sday) && $crank !~ /$srank/) {  
	 ...
	 if ($w{Rule} !~ /omit.*? commemoratio/i && $climit1960  && ($w{Rule} !~ /No commemoratio/i)) {
       ...
       if (($climit1960 > 1 && $hora =~ /laudes/i) || $climit1960 < 2) {
         ...
         $cvespera = $svesp;
         $comrank = $srank[2]; 
         if ($crank[2] >= 3 || ($trank[2] == 5 && $crank[2] >= 2) ) {$commemoratio1 = $cname;} 
       } 
	 } else {
        $dayname[2] = '';
        $commemoratio = '';
...
sub precedence {	  
  ...
  if ($commemoratio1) {   
    %commemoratio1 = %{officestring("$datafolder/$lang1/$commemoratio1")};
  }
  ...
  if ($version =~ /(1955|1960)/ && $month == 1 && $day > 6 && $day < 13 && $winner{Rank} =~ /Die/i  && exists($scriptura{Rank})) 
  ...
  if ($dayofweek == 0 && $month == 12 && $day == 24) {
...
sub transfered { 
  ...
  foreach $key (keys %transfer) {
	if ($key !~ /dirge/i && $transfer{$key} !~ /$key/ && $transfer{$key} =~ /$str/i && $transfer{$key} !~ /$str\s*a/i  ) 
       ##&& $transfer{$key} !~ /\v\s*$/i) 
	   {return 1;}
  }		 
  if (%transferspec) {
