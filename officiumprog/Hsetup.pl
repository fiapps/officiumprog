...
sub setuptable {
  ...
  if ($name =~ /Chant/i && $Hk > 1) {
    {setuprut('Chant', getsetuppar('Chant'), $dialog{'helpchant'}, \&setupproc)}
  } else {
    setuprut('parameters',getsetuppar('parameters'),$dialog{'helpparam'},\&setupproc);
  }
}
...
sub fontgener {
  ...
  ($titlecolor, $frmc, $titlefont) = getcolors($titlelargeframe);
...
sub restore_original {
  my @a;
  if (open(INP, "$datafolder/Hhoras_orig.setup") && open(OUT,  ">$datafolder/Hhoras.setup")) {
    my @a = <INP>;
    close INP;
    foreach $line (@a) {print OUT $line;}
    close OUT;
    %setup = %{setupstring("$datafolder/Hhoras.setup")};
    eval $setup{general};
    eval $setup{'Param'};	      
    eval $setup{'Colors'};
 ...
 else {error("FATAL ERROR Hhoras_orig.setup is missing");}
}   
