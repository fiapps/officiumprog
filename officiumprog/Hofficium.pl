...
our ($notes,$voicecolumn, $doline, $keyfreq, $dofreq, $basetime,$voweltime, $vowelmod, $mono);
...
for ($i = 0; $i < @names; $i++) {
  $item = $names[$i];  
  my $label = $bframe2->Label(-text=>"$names[$i]")->grid(-column=>$i, -row=>0, -padx=>$padx, -pady=>$pady);
  configure($label, $framecolor, $labelfont, $titlecolor);
}
...
@versions = ('pre Trident Monastic', 'Trident 1570', 'Trident 1910', 'Divino Afflatu', 'Reduced 1955', 'Rubrics 1960', '1960 Newcalendar');
...
$bframe3->Button(-text=>'Help',
  ...
  my $label = $bottomframe->Label(-textvariable=>\$error)->pack(-side=>'top', -padx=>$padx);
  configure($label, $framecolor, $labelfont, $titlecolor);
  $label = $bottomframe->Label(-textvariable=>\$message, justify=>'left')->pack(-side=>'top', -padx=>$padx);
  configure($label, $framecolor, $labelfont, $titlecolor);
...
sub generate {
  ...
  while ($maxnum < $numofdays) {
	...
	if (!$onefile || $voicecolumn =~ /chant/i) {
  ...
  if (opendir(DIR, "$datafolder/Latin/Tabulae")) {
	...
	foreach my $fname (@a) {
	  if ($fname =~ /(Tr|Str)(M|1570|1910|DA|1960|Newcal)([0-9]+)/ && ($3 < 2008 || $3 > 2015)) 
	    {if (unlink("$datafolder/Latin/Tabulae/$1$2$3.txt") == 0) 
		   {print "$datafolder/Latin/Tabulae/$1$2$3.txt cannot delete\n";}}
    } 
  }
